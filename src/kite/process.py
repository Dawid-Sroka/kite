...
class ProcessImage:
    def __init__(self, pid, cpu_context: CPUContext, process_routine):
        ...
        self.zombies = []
        self.process_routine_stack = deque()
        self.process_routine_stack.append(process_routine)
        self.signal_received = -1
        self.command = ""

    @property
    def process_routine(self):
        return self.process_routine_stack[-1]

    @process_routine.setter
    def process_routine(self, process_routine):
        self.process_routine_stack[-1] = process_routine
    ...
    def pop_subroutine(self):
        self.cpu_context_stack.pop()
        self.process_routine_stack.pop()
...
class OpenFileObject():
    def __init__(self, file_name, file_struct = None):
...
class RegularFile(OpenFileObject):
    def __init__(self, file_name, file_descriptor):
        super().__init__(file_name, file_descriptor)
